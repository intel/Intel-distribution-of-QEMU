...
static GtkWidget *gd_create_menu_view(GtkDisplayState *s, DisplayOptions *opts)
{
    ...
    if (!s->opts->u.gtk.has_input || s->opts->u.gtk.input) {
        ...
        s->grab_on_hover_item = gtk_check_menu_item_new_with_mnemonic(
                                                          _("Grab On _Hover"));
